...
const MyWishList = () => {
    ...
    const handlePayment = (id, price, title) => {
        Swal.fire({
            title: "Proceed to Payment?",
            text: `You are about to pay $${price} for "${title}".`,
            icon: "info",
            showCancelButton: true,
                    ...
                    <tbody>
                        {wishlist.map((item, index) => (
                            ...
                            >
                                ...
                                {/* Wishlist Item Name */}
                                <td className="p-4 border border-gray-300">{item.title || "No title available"}</td>

                                {/* Price */}
                                <td className="p-4 border border-gray-300">${item.price || "N/A"}</td>

                                {/* Actions: Payment & Delete */}
                                <td className="p-4 border border-gray-300 text-center flex justify-center gap-4">
                                    {/* Pay Now Button */}
                                    <button
                                        onClick={() => handlePayment(item.id, item.price, item.title)}
